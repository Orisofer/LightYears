...
namespace ly
{
    ...
    {
        if (other == nullptr || other->IsPendingDestroyed())
        {
            return;
        }
        ...
        weak<PlayerSpaceship> playerSpaceship = PlayerManager::Get().GetPlayer()->GetSpaceship();

        if (!playerSpaceship.expired() || playerSpaceship.lock()->IsPendingDestroyed())
        {
